...
window.sendFile = function sendFile() {
  ...
  dataChannel.onopen = () => {
    console.log("Sender: DataChannel open. Sending meta");
    ...
    fileReader.onload = e => {
      ...
      if (offset < file.size) readSlice(offset);
        ...
        dataChannel.close();
        console.log("Sender: File sent, DataChannel closed");
      }
    };
  ...
  peerConnection.onicecandidate = e => {
    if (e.candidate) {
      console.log("Sender: ICE candidate", e.candidate);
...
window.onload = function() {
  ...
  if (params.has('room')) {
    ...
    console.log("Receiver: PeerConnection created");
    ...
    peerConnection.ondatachannel = ev => {
      ...
      dataChannel.onmessage = event => {
        if (typeof event.data === 'string' && event.data.includes('__meta')) {
          ...
          console.log("Receiver: Meta received", fileMeta);
          return;
        }
      ...
      dataChannel.onclose = () => {
        console.log("Receiver: DataChannel closed, reconstructing file");
    ...
    peerConnection.onicecandidate = e => {
      if (e.candidate) {
        console.log("Receiver: ICE candidate", e.candidate);
        socket.emit('signal', { room: currentRoom, data: { candidate: e.candidate } });
      }
    };
  }
